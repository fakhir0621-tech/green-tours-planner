  ...
  return (
    <div style={{ textAlign: "center", padding: "60px 0" }}>
      ...
      <h3 style={{
        ...
        fontSize: "20px", color: "var(--gray-800)", marginBottom: "8px",
      }}>{title}</h3>
...
export default function Account() {
  ...
  // Wishlist from context — this is the source of truth
  const {
    wishlistTours,
    isWishlisted,
  ...
  // Load AI recs only when user opens that section
  useEffect(() => {
    if (activeSection === "ai" && !aiLoaded && allTours.length > 0) {
      loadAIRecs();
    }
  ...
  try {
    const userId = user._id || user.id;
    // Try user-specific endpoint first, fall back to filtering all reviews
    ...
    if (res.ok) {
      ...
      // Fallback — fetch all reviews and filter by user
      ...
      const mine = all.filter(r =>
        (r.userId?._id || r.userId || r.user) === userId
      );
      setReviews(mine);
    }
  } catch {
  ...
  // ---- AI RECOMMENDATIONS — rule-based, no dummy output ----
  const loadAIRecs = async () => {
    ...
    try {
      ...
      const bookedTourIds = bookings
        .map(b => b.tourId?._id || b.tourId)
        .filter(Boolean);
      const bookedCategories = bookings
        .map(b => b.tourId?.category)
        .filter(Boolean);
      const bookedPrices = bookings
        .map(b => b.tourId?.price || 0)
        .filter(Boolean);
      const avgPrice = bookedPrices.length > 0
        ? bookedPrices.reduce((a, b) => a + b, 0) / bookedPrices.length
        : null;

      // Candidates = tours not already booked
      const candidates = allTours.filter(
        t => !bookedTourIds.includes(t._id || t.id)
      );

      // Nothing to recommend if no tours available
      ...
      // Try backend AI route first (if you've set up recommendationRoutes.js)
      let recs = [];
      try {
        ...
        if (res.ok) {
          ...
          if (data.ids && Array.isArray(data.ids)) {
            recs = data.ids
              .map(id => candidates.find(t => (t._id || t.id) === id))
              .filter(Boolean);
          }
        }
      } catch {
        // Backend not set up yet — fall through to rule-based
      }

      // Rule-based fallback if backend gave nothing
      if (recs.length < 3) {
        const preferredCategories = bookedCategories.length > 0
          ? bookedCategories
          : (visited.length > 0
            ? candidates
                .filter(t => visited.includes(t._id || t.id))
                .map(t => t.category)
                .filter(Boolean)
            : []);

        const scored = candidates
          .filter(t => !recs.find(r => (r._id || r.id) === (t._id || t.id)))
          .map(t => {
            let score = 0;
            if (preferredCategories.includes(t.category)) score += 4;
            if (visited.includes(t._id || t.id)) score += 3;
            if (avgPrice !== null) {
              const diff = Math.abs((t.price || 0) - avgPrice);
              if (diff < 300) score += 3;
              else if (diff < 800) score += 2;
              else if (diff < 1500) score += 1;
            }
            // Boost high-rated tours
            if ((t.rating || 0) >= 4.8) score += 1;
            return { ...t, score };
          })
        ...
        recs = [...recs, ...scored.slice(0, needed)];
      }
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      await fetch(`${BASE_URL}/users/profile/${user._id || user.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleCancelBooking = async (id) => {
    ...
    // Remove cancelled booking from list entirely
    setBookings(prev => prev.filter(b => b._id !== id));
    setCancellingId(null);
  };

  // ---- WISHLIST REMOVE — uses context so heart on cards updates too ----
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      const saved = data.review || data.data || {
        ...
        ...reviewForm,
        tourId: allTours.find(t => t._id === reviewForm.tourId) || { title: "Tour" },
      ...
      // Optimistic add even if backend fails
      const optimistic = {
        _id: editingReviewId || Date.now().toString(),
        ...reviewForm,
        tourId: allTours.find(t => t._id === reviewForm.tourId) || { title: "Tour" },
      ...
      <div style={{
        ...
        <div style={{
          ...
          {[
            ...
            { num: wishlistTours.length, label: "Wishlist" },
        ...
        <div style={{
          ...
          {SECTIONS.map((s, i) => (
            <button key={s.key}
              ...
              style={{
                ...
                background: activeSection === s.key ? "var(--green-50)" : "white",
                color: activeSection === s.key ? "var(--green-700)" : "var(--gray-600)",
                border: "none",
                borderBottom: i < SECTIONS.length - 1
                  ? "1px solid var(--gray-50)" : "none",
                borderLeft: activeSection === s.key
          ...
          <button
            ...
            style={{
              ...
              border: "none", borderTop: "1px solid var(--gray-100)",
              fontSize: "13px", fontWeight: "500",
              textAlign: "left", cursor: "pointer",
          ...
          {activeSection === "profile" && (
            <SectionCard>
              ...
              <div style={{
                ...
                <div>
                  <p style={{ fontSize: "14px", fontWeight: "600", color: "var(--gray-800)", marginBottom: "6px" }}>
                    Profile Photo
                  </p>
              ...
              <form onSubmit={handleSaveProfile}>
                <div style={{
                  ...
                  <div>
                    <label style={{ display: "block", fontSize: "13px", fontWeight: "600", color: "var(--gray-600)", marginBottom: "7px" }}>
                      Full Name *
                    </label>
                    <input
                ...
                <button type="submit" disabled={savingProfile} style={{
                  background: savingProfile ? "var(--green-400)" : "var(--green-600)",
            ...
            <SectionCard>
              ...
              {bookingsLoading ? (
                ...
                <EmptyState
                  ...
                  desc="Browse our tours and book your first adventure!"
                  link="/tours"
  ...
  const PAY_METHOD_LABELS = {
    easypaisa: { label: "EasyPaisa", color: "#00a651", bg: "#e8f8f0" },
    jazzcash:  { label: "JazzCash",  color: "#cc0000", bg: "#fff0f0" },
    bank:      { label: "Bank Transfer", color: "#1a56db", bg: "#eff6ff" },
  };
  const pm = PAY_METHOD_LABELS[payMethod] || null;

  return (
    <div key={b._id} style={{
      border: `1px solid ${status === "confirmed" ? "#86efac" : status === "rejected" ? "#fecaca" : "var(--gray-100)"}`,
      ...
      <div style={{
          ...
          {status === "rejected" && (
            <div style={{
              ...
              <span style={{
                ...
                fontWeight: "700", background: "#dc2626",
        ...
        <div style={{ padding: "18px 20px" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
              ...
              <span style={{
                ...
                background: payStatus === "Verified" ? "#dcfce7" : payStatus === "Rejected" ? "#fee2e2" : "#fef9c3",
                color: payStatus === "Verified" ? "#16a34a" : payStatus === "Rejected" ? "#dc2626" : "#ca8a04",
              }}>
                💳 Payment: {payStatus}
              </span>
              {invoiceNo && (
          ...
          <h3 style={{
            ...
            color: "var(--gray-800)", marginBottom: "10px",
            lineHeight: "1.3",
          }}>
            {tourName}
          </h3>
          ...
          <div style={{
            ...
            {[
              { icon: "📍", val: tourLocation },
              { icon: "🕐", val: tourDuration },
              { icon: "📅", val: travelDate
                  ? new Date(travelDate).toLocaleDateString("en-PK", {
                      month: "short", day: "numeric", year: "numeric",
                    })
                  : "Date N/A"
              },
              { icon: "👥", val: `${guestCount} guest${guestCount > 1 ? "s" : ""}` },
            ].map(m => (
          ...
          {/* PENDING NOTICE */}
          {status === "pending" && payStatus === "Pending" && (
            <div style={{
              marginTop: "10px",
              background: "#fef9c3", border: "1px solid #fde68a",
              borderRadius: "8px", padding: "8px 12px",
              ...
              ⏳ Awaiting payment verification by admin — usually within 24 hours.
            </div>
          )}
        </div>
      </div>
          ...
          {/* ---- MY WISHLIST — reads from context, no backend fetch needed ---- */}
            ...
            <SectionCard>
              ...
              {wishlistTours.length === 0 ? (
                <EmptyState
                  ...
                  desc="Click the red 'Add to Wishlist' button on any tour card to save it here."
                  link="/tours"
                  linkLabel="Browse Tours"
                />
              ) : (
                <div style={{
                  ...
                  {wishlistTours.map(tour => {
                    ...
                    const image = tour.images?.[0] || tour.photo || tour.image ||
                      ...
                      >
                        ...
                        <div style={{ padding: "14px", flex: 1, display: "flex", flexDirection: "column" }}>
                          ...
                          <div style={{
                            ...
                            <button
                              onClick={() => handleRemoveWishlist(tourId)}
                              disabled={removingId === tourId}
                              style={{
                                background: removingId === tourId ? "var(--gray-100)" : "#fef2f2",
                                color: removingId === tourId ? "var(--gray-400)" : "#dc2626",
                                border: "1.5px solid #fecaca",
                                padding: "5px 12px", borderRadius: "50px",
                                fontSize: "11px", fontWeight: "600",
                                cursor: removingId === tourId ? "not-allowed" : "pointer",
                                transition: "all 0.2s",
                              }}
                              onMouseEnter={e => {
                                if (removingId !== tourId) {
                                  e.currentTarget.style.background = "#dc2626";
                                  e.currentTarget.style.color = "white";
                                }
                              }}
          ...
          {activeSection === "ai" && (
            <SectionCard>
              ...
              <p style={{ color: "var(--gray-400)", fontSize: "14px", marginBottom: "24px" }}>
                Recommendations based on tours you've visited and booked.
              </p>

              {aiLoading ? (
                <LoadingSpinner label="Finding tours you'll love..." />
              ) : aiRecs.length === 0 ? (
                <EmptyState
                  icon="✨"
                  title="No recommendations yet"
                  desc="Browse and click on a few tours first — recommendations are based on what you explore and book."
                  link="/tours"
                  linkLabel="Browse Tours to Get Started"
                />
              ) : (
                <>
                  <div style={{
                    ...
                    🤖 Based on your browsing history and booking preferences
                  </div>
                  ...
                  <div style={{ textAlign: "center", marginTop: "24px" }}>
                    <button
                      onClick={() => { setAiLoaded(false); setAiRecs([]); loadAIRecs(); }}
                      style={{
            ...
            <SectionCard>
              <div style={{
                ...
                <button
                  ...
                  style={{
                    background: showReviewForm ? "var(--gray-100)" : "var(--green-600)",
                    color: showReviewForm ? "var(--gray-600)" : "white",
                    border: "none",
              ...
              {showReviewForm && (
                <div style={{
                  ...
                  <h3 style={{
                    ...
                    color: "var(--gray-800)", marginBottom: "18px",
                  }}>
                    {editingReviewId ? "Edit Review" : "Write a New Review"}
                  </h3>
                  <form onSubmit={handleReviewSubmit}>
                    <div style={{ marginBottom: "16px" }}>
                      ...
                      <select
                        value={reviewForm.tourId}
                        onChange={e => setReviewForm({ ...reviewForm, tourId: e.target.value })}
                        disabled={!!editingReviewId}
                        style={{
                          ...inputStyle,
                          background: editingReviewId ? "var(--gray-100)" : "white",
                          cursor: editingReviewId ? "not-allowed" : "pointer",
                        }}
                      >
                    ...
                    <div style={{ marginBottom: "16px" }}>
                      <label style={{ display: "block", fontSize: "13px", fontWeight: "600", color: "var(--gray-600)", marginBottom: "10px" }}>
                        Rating *
                      </label>
                    ...
                    <div style={{ display: "flex", gap: "10px" }}>
                      <button type="submit" disabled={submittingReview} style={{
                        background: submittingReview ? "var(--green-400)" : "var(--green-600)",
                        color: "white", border: "none",
                        padding: "11px 28px", borderRadius: "50px",
                        fontSize: "14px", fontWeight: "600",
              ...
              {reviewsLoading ? (
                ...
                <EmptyState
                  ...
                  desc="Booked a tour? Share your experience to help other travelers."
                />
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                  {reviews.map(review => {
                    ...
                    return (
                      ...
                      >
                        <img
                          src={image}
                          alt={tour.tourName || tour.title || "Tour"}
                          style={{ width: "100%", height: "100%", objectFit: "cover" }}
                        />
                        <div style={{ padding: "16px 20px" }}>
                          ...
                          <div style={{ marginBottom: "10px" }}>
                            <Stars value={review.rating || 5} size={16} />
                          </div>
                          <p style={{
                          ...
                          <div style={{ display: "flex", gap: "8px" }}>
                            ...
                            <button
                              ...
                              style={{
                                ...
                                cursor: deletingReviewId === review._id
                                  ? "not-allowed" : "pointer",
                              }}
                            >
