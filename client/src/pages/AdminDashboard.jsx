...
function BarChart({ data, color = "#16a34a", height = 160, label = "" }) {
  ...
  return (
    <div>
      {label && (
        <p style={{ fontSize: "13px", fontWeight: "600", color: "var(--gray-600)", marginBottom: "12px" }}>
          {label}
        </p>
      )}
      <div style={{ display: "flex", alignItems: "flex-end", gap: "6px", height: `${height}px` }}>
        {data.map((d, i) => (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "4px", height: "100%" }}>
            ...
            <span style={{ fontSize: "10px", color: "var(--gray-400)", textAlign: "center", whiteSpace: "nowrap", overflow: "hidden", maxWidth: "100%" }}>
              {d.label}
            </span>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: "20px", flexWrap: "wrap" }}>
      <svg width={size} height={size} style={{ flexShrink: 0 }}>
        {segments.map((seg, i) => {
          ...
          cumulative += angle;
          return (
            <path key={i} d={arcPath(start, start + angle - 0.5)}
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "12px" }}>
        <div style={{ width: "40px", height: "40px", borderRadius: "10px", background: color + "18", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "20px" }}>
          {icon}
        </div>
        <div>
          <p style={{ fontSize: "12px", color: "var(--gray-400)", fontWeight: "500" }}>{label}</p>
          <p style={{ fontSize: "22px", fontWeight: "700", color: color, fontFamily: "'Playfair Display', serif", lineHeight: 1 }}>
            {value}
          </p>
        </div>
      </div>
...
function TourFormModal({ editingTour, onClose, onSaved, token }) {
  ...
  const [form, setForm] = useState(() => {
    if (isEdit) {
      return {
        ...
        availableSeats:  String(editingTour.availableSeats || "10"),
        virtualTourLink: editingTour.virtualTourLink || "",
        images:          editingTour.images?.length > 0
          ? [...editingTour.images, "", ""].slice(0, 3)
          : ["", "", ""],
  ...
  const inputStyle = {
    ...
    border: "1.5px solid #e5e7eb", borderRadius: "9px", fontSize: "14px",
    fontFamily: "'DM Sans', sans-serif", color: "#1f2937", background: "white",
    outline: "none", boxSizing: "border-box", transition: "border-color 0.2s",
  };
  const labelStyle = {
    display: "block", fontSize: "12px", fontWeight: "600", color: "#4b5563",
    marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.5px",
  };
  const setField = (key, val) => { setForm(prev => ({ ...prev, [key]: val })); setError(""); };
  const setImage = (index, val) => {
    const imgs = [...form.images]; imgs[index] = val;
    setForm(prev => ({ ...prev, images: imgs }));
  };
  const addItineraryDay = () => {
    setForm(prev => ({ ...prev, itinerary: [...prev.itinerary, { day: prev.itinerary.length + 1, description: "" }] }));
  };
  const removeItineraryDay = (index) => {
    const updated = form.itinerary.filter((_, i) => i !== index).map((d, i) => ({ ...d, day: i + 1 }));
    setForm(prev => ({ ...prev, itinerary: updated }));
  };
  const setItineraryDesc = (index, val) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      tourName: form.tourName.trim(), location: form.location.trim(),
      price: Number(form.price), duration: form.duration.trim(),
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{
      ...
      zIndex: 9999, padding: "20px", backdropFilter: "blur(3px)",
    }}>
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <p style={{ fontSize: "10px", letterSpacing: "3px", color: "#4ade80", fontWeight: "600", textTransform: "uppercase", marginBottom: "4px" }}>
              {isEdit ? "EDITING TOUR" : "NEW TOUR"}
            </p>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "20px", fontWeight: "700", color: "white" }}>
              {isEdit ? editingTour.tourName : "Add a New Tour"}
            </h2>
          </div>
        ...
        <div style={{ display: "flex", background: "#f9fafb", borderBottom: "1px solid #e5e7eb", flexShrink: 0, overflowX: "auto" }}>
          {SECTIONS.map(s => (
            <button key={s.key} onClick={() => setActiveSection(s.key)} style={{
              flex: 1, minWidth: "fit-content", padding: "12px 16px", border: "none",
              borderBottom: activeSection === s.key ? "2px solid #16a34a" : "2px solid transparent",
              background: activeSection === s.key ? "#f0fdf4" : "transparent",
              color: activeSection === s.key ? "#16a34a" : "#6b7280",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                    <div style={{ gridColumn: "1 / -1" }}>
                      ...
                      <input type="text" value={form.tourName} onChange={e => setField("tourName", e.target.value)} placeholder="e.g. Hunza Valley Adventure" style={inputStyle} onFocus={e => e.target.style.borderColor = "#16a34a"} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
                    </div>
                    <div>
                      <label style={labelStyle}>Location *</label>
                      <input type="text" value={form.location} onChange={e => setField("location", e.target.value)} placeholder="e.g. Hunza, Pakistan" style={inputStyle} onFocus={e => e.target.style.borderColor = "#16a34a"} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input type="number" value={form.price} onChange={e => setField("price", e.target.value)} placeholder="e.g. 25000" min="0" style={inputStyle} onFocus={e => e.target.style.borderColor = "#16a34a"} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
                    </div>
                    <div>
                      <label style={labelStyle}>Duration</label>
                      <input type="text" value={form.duration} onChange={e => setField("duration", e.target.value)} placeholder="e.g. 5 Days" style={inputStyle} onFocus={e => e.target.style.borderColor = "#16a34a"} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
                    </div>
                    <div>
                      <label style={labelStyle}>Available Seats</label>
                      <input type="number" value={form.availableSeats} onChange={e => setField("availableSeats", e.target.value)} placeholder="e.g. 20" min="1" style={inputStyle} onFocus={e => e.target.style.borderColor = "#16a34a"} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeSection === "details" && (
                <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
                  <div>
                    ...
                    <textarea value={form.description} onChange={e => setField("description", e.target.value)} placeholder="Describe the tour experience..." rows={7} style={{ ...inputStyle, resize: "vertical", lineHeight: "1.7" }} onFocus={e => e.target.style.borderColor = "#16a34a"} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
                    <p style={{ fontSize: "11px", color: "#9ca3af", marginTop: "5px" }}>{form.description.length} characters</p>
                  </div>
                  <div>
                    <label style={labelStyle}>Virtual Tour Link (optional)</label>
                    <input type="url" value={form.virtualTourLink} onChange={e => setField("virtualTourLink", e.target.value)} placeholder="https://..." style={inputStyle} onFocus={e => e.target.style.borderColor = "#16a34a"} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
                  </div>
                </div>
              )}
              ...
              {activeSection === "images" && (
                <div>
                  <p style={{ fontSize: "13px", color: "#6b7280", marginBottom: "20px", lineHeight: "1.7", background: "#f0fdf4", padding: "12px 14px", borderRadius: "8px", border: "1px solid #bbf7d0" }}>
                    💡 Paste direct image URLs. Use Unsplash or Imgur for free hosting.
                  </p>
                  <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                    {form.images.map((img, index) => (
                      <div key={index}>
                        ...
                        {img.trim() !== "" && (
                          <div style={{ marginTop: "8px", height: "100px", borderRadius: "8px", overflow: "hidden", border: "1px solid #e5e7eb" }}>
                            <img src={img} alt={`Preview ${index + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { e.target.style.display = "none"; e.target.parentElement.innerHTML = '<div style="height:100%;display:flex;align-items:center;justify-content:center;background:#fef2f2;color:#dc2626;font-size:12px;">⚠️ Invalid image URL</div>'; }} />
                          </div>
                        )}
                      </div>
                    ))}
              ...
              {activeSection === "itinerary" && (
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                    <div>
                      <p style={{ fontSize: "14px", fontWeight: "600", color: "#1f2937" }}>Day-by-Day Itinerary</p>
                      <p style={{ fontSize: "12px", color: "#9ca3af", marginTop: "2px" }}>Optional</p>
                    </div>
                  ...
                  {form.itinerary.length === 0 && (
                    <div style={{ textAlign: "center", padding: "40px", background: "#f9fafb", borderRadius: "12px", border: "1.5px dashed #e5e7eb" }}>
                      ...
                      <p style={{ fontSize: "14px", color: "#9ca3af" }}>No itinerary yet. Click "+ Add Day".</p>
                    </div>
                  )}
                  <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                    {form.itinerary.map((day, index) => (
                      <div key={index} style={{ display: "flex", gap: "12px", alignItems: "flex-start" }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <textarea value={day.description} onChange={e => setItineraryDesc(index, e.target.value)} placeholder={`Day ${day.day} — describe activities...`} rows={2} style={{ ...inputStyle, resize: "vertical", lineHeight: "1.6" }} onFocus={e => e.target.style.borderColor = "#16a34a"} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
                        </div>
                        {form.itinerary.length > 1 && (
                          <button type="button" onClick={() => removeItineraryDay(index)} style={{ background: "#fef2f2", color: "#dc2626", border: "1.5px solid #fecaca", width: "32px", height: "32px", borderRadius: "50%", fontSize: "14px", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", marginTop: "2px" }}>✕</button>
                        )}
                      </div>
                    ))}
                  </div>
...
function UserActionModal({ targetUser, action, onClose, onDone, token }) {
  ...
  const ENDPOINT_MAP = {
    ban: `/users/ban/`, unban: `/users/unban/`, suspend: `/users/suspend/`,
    unsuspend: `/users/unsuspend/`, flag: `/users/flag/`, unflag: `/users/unflag/`,
    reset: `/users/reset/`, notes: `/users/notes/`, role: `/users/role/`, delete: `/users/`,
  };
    ...
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{
      ...
      <div style={{ background: "white", borderRadius: "18px", width: "100%", maxWidth: "460px", boxShadow: "0 24px 60px rgba(0,0,0,0.25)", overflow: "hidden" }}>
        <div style={{ background: cfg.color + "12", borderBottom: `1px solid ${cfg.color}25`, padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <div>
              ...
              <p style={{ fontSize: "12px", color: "#6b7280", marginTop: "1px" }}>{targetUser?.name || targetUser?.email}</p>
            </div>
          </div>
        ...
        <div style={{ padding: "22px 24px" }}>
          ...
          {!cfg.needsReason && !cfg.needsDays && !cfg.needsRole && !cfg.needsNotes && (
            <p style={{ fontSize: "14px", color: "#374151", lineHeight: "1.7" }}>
              {action === "delete" ? `Permanently delete ${targetUser?.name}'s account? This cannot be undone.`
                : action === "reset" ? `Clear all bans, suspensions, and flags for ${targetUser?.name}?`
                : action === "unban" ? `Lift the ban for ${targetUser?.name}?`
                : action === "unsuspend" ? `End the suspension for ${targetUser?.name}?`
                : `Remove the flag from ${targetUser?.name}?`}
            </p>
          )}
          {cfg.needsReason && (
            <div style={{ marginBottom: "16px" }}>
              <label style={{ display: "block", fontSize: "12px", fontWeight: "600", color: "#4b5563", marginBottom: "6px", textTransform: "uppercase" }}>Reason *</label>
              <textarea value={reason} onChange={e => { setReason(e.target.value); setError(""); }} rows={3}
                style={{ width: "100%", padding: "10px 13px", border: "1.5px solid #e5e7eb", borderRadius: "9px", fontSize: "14px", fontFamily: "'DM Sans', sans-serif", color: "#1f2937", background: "white", outline: "none", resize: "none", lineHeight: "1.6", boxSizing: "border-box" }}
                onFocus={e => e.target.style.borderColor = cfg.color} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
            </div>
          )}
          {cfg.needsDays && (
            <div style={{ marginBottom: "16px" }}>
              <label style={{ display: "block", fontSize: "12px", fontWeight: "600", color: "#4b5563", marginBottom: "8px", textTransform: "uppercase" }}>Duration</label>
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {[{ val: "1", label: "1 Day" }, { val: "3", label: "3 Days" }, { val: "7", label: "1 Week" }, { val: "14", label: "2 Weeks" }, { val: "30", label: "1 Month" }].map(d => (
                  <button key={d.val} type="button" onClick={() => setDays(d.val)} style={{ padding: "7px 14px", borderRadius: "50px", fontSize: "12px", fontWeight: "600", border: days === d.val ? `2px solid ${cfg.color}` : "1.5px solid #e5e7eb", background: days === d.val ? cfg.color + "15" : "white", color: days === d.val ? cfg.color : "#6b7280", cursor: "pointer" }}>{d.label}</button>
                ))}
              </div>
            </div>
          )}
          {cfg.needsRole && (
            <div style={{ marginBottom: "16px" }}>
              <label style={{ display: "block", fontSize: "12px", fontWeight: "600", color: "#4b5563", marginBottom: "8px", textTransform: "uppercase" }}>New Role</label>
              <div style={{ display: "flex", gap: "10px" }}>
                {["traveler", "admin"].map(r => (
                  <button key={r} type="button" onClick={() => setNewRole(r)} style={{ flex: 1, padding: "12px", borderRadius: "10px", border: newRole === r ? `2px solid ${cfg.color}` : "1.5px solid #e5e7eb", background: newRole === r ? cfg.color + "12" : "white", color: newRole === r ? cfg.color : "#6b7280", fontSize: "13px", fontWeight: "600", cursor: "pointer", textTransform: "capitalize" }}>
                    {r === "admin" ? "👑 Admin" : "🧳 Traveler"}
                  </button>
                ))}
              </div>
            </div>
          ...
          {cfg.needsNotes && (
            <div style={{ marginBottom: "8px" }}>
              <label style={{ display: "block", fontSize: "12px", fontWeight: "600", color: "#4b5563", marginBottom: "6px", textTransform: "uppercase" }}>Internal Notes</label>
              <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4}
                style={{ width: "100%", padding: "10px 13px", border: "1.5px solid #e5e7eb", borderRadius: "9px", fontSize: "14px", fontFamily: "'DM Sans', sans-serif", color: "#1f2937", background: "white", outline: "none", resize: "vertical", lineHeight: "1.6", boxSizing: "border-box" }}
                onFocus={e => e.target.style.borderColor = cfg.color} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
            </div>
          )}
        </div>
...
export default function AdminDashboard() {
  ...
  const [blockedUsers, setBlockedUsers]           = useState(() => {
    try { return JSON.parse(localStorage.getItem("gt_blocked_users") || "[]"); }
    catch { return []; }
  });
  ...
  const updateBookingStatus = async (id, status) => {
    try {
      await fetch(`${BASE_URL}/bookings/status/${id}`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ status }) });
  ...
  const toggleBlockUser = (userId, userName) => {
    ...
    localStorage.setItem("gt_blocked_users", JSON.stringify(updated));
  };
        ...
        {activeTab === "overview" && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "20px", marginBottom: "36px" }}>
              {[
                { icon: "👥", label: "Total Users",    val: stats.totalUsers,    color: "#0891b2" },
                { icon: "🗺️", label: "Total Tours",    val: stats.totalTours,    color: "#7c3aed" },
                { icon: "📋", label: "Total Bookings", val: stats.totalBookings, color: "#db6b1f" },
                { icon: "💰", label: "Total Revenue",  val: `Rs. ${Number(stats.totalRevenue).toLocaleString()}`, color: "#16a34a" },
              ].map((s) => (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <div>
                    <p style={{ fontSize: "14px", fontWeight: "700", color: "#92400e" }}>
                      {bannedCount > 0 && `${bannedCount} banned`}{bannedCount > 0 && (suspendedCount > 0 || flaggedCount > 0) && " · "}
                      {suspendedCount > 0 && `${suspendedCount} suspended`}{suspendedCount > 0 && flaggedCount > 0 && " · "}
                      {flaggedCount > 0 && `${flaggedCount} flagged`}
                    </p>
                    <p style={{ fontSize: "12px", color: "#a16207" }}>Review moderation actions for user accounts</p>
                  </div>
                </div>
            ...
            {pendingPayments.length > 0 && (
              <div style={{ background: "#fef9c3", border: "1px solid #fde68a", borderRadius: "12px", padding: "16px 20px", marginBottom: "28px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <div>
                    <p style={{ fontSize: "14px", fontWeight: "700", color: "#92400e" }}>{pendingPayments.length} payment{pendingPayments.length > 1 ? "s" : ""} awaiting verification</p>
                    <p style={{ fontSize: "12px", color: "#a16207" }}>Review screenshots and approve or reject bookings</p>
                  </div>
                </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {bookings.slice(0, 5).map((b) => {
                      ...
                      const pm = PAYMENT_METHOD_INFO[b.paymentMethod] || {};
                      return (
                        <tr key={b._id} onMouseEnter={e => e.currentTarget.style.background = "var(--gray-50)"} onMouseLeave={e => e.currentTarget.style.background = "white"}>
                          ...
                          <td style={tdStyle}>{b.tour?.tourName || "N/A"}</td>
                          <td style={{ ...tdStyle, fontWeight: "600", color: "var(--green-700)" }}>Rs. {Number(b.totalPrice || 0).toLocaleString()}</td>
                          <td style={tdStyle}>{pm.name ? <span style={{ background: pm.bg, color: pm.color, padding: "2px 10px", borderRadius: "50px", fontSize: "11px", fontWeight: "600" }}>{pm.icon} {pm.name}</span> : "—"}</td>
                          <td style={tdStyle}><span style={{ background: sc.bg, color: sc.color, padding: "3px 12px", borderRadius: "50px", fontSize: "12px", fontWeight: "600", textTransform: "capitalize" }}>{status}</span></td>
                        </tr>
                      );
                    })}
        ...
        {activeTab === "moderation" && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "16px", marginBottom: "28px" }}>
              {[
                { label: "Total Users",  val: users.length,    color: "#0891b2", bg: "#e0f2fe", icon: "👥" },
                { label: "Active",       val: users.filter(u => !u.isBanned && !u.isSuspended && !u.isFlagged).length, color: "#16a34a", bg: "#dcfce7", icon: "✅" },
                { label: "Banned",       val: bannedCount,     color: "#dc2626", bg: "#fee2e2", icon: "🚫" },
                { label: "Suspended",    val: suspendedCount,  color: "#d97706", bg: "#fef9c3", icon: "⏸️" },
                { label: "Flagged",      val: flaggedCount,    color: "#7c3aed", bg: "#ede9fe", icon: "🚩" },
                { label: "Admins",       val: users.filter(u => u.role === "admin").length, color: "#1d4ed8", bg: "#eff6ff", icon: "👑" },
              ].map(s => (
            ...
            {filteredForModeration.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px 0", background: "white", borderRadius: "16px", border: "1px solid var(--gray-100)" }}>
                ...
                <p style={{ color: "var(--gray-400)", fontSize: "15px" }}>No users match your filter.</p>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        {activeTab === "payments" && (
          <div style={{ background: "white", border: "1px solid var(--gray-100)", borderRadius: "16px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--gray-100)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
              <div>
                <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "18px", fontWeight: "700", color: "var(--gray-800)", marginBottom: "4px" }}>Payment Verification</h3>
                <p style={{ fontSize: "13px", color: "var(--gray-400)" }}>Review payment screenshots and approve or reject bookings.</p>
              </div>
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            ...
            {bookings.filter(b => b.transactionId).length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px 0" }}>
                <div style={{ fontSize: "48px", marginBottom: "12px" }}>💳</div>
                <p style={{ color: "var(--gray-400)", fontSize: "15px" }}>No payment bookings yet.</p>
              </div>
            ) : (
              <div>
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "16px", flexWrap: "wrap", gap: "12px" }}>
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px", flexWrap: "wrap" }}>
                            ...
                            <span style={{ fontSize: "11px", fontWeight: "700", padding: "2px 10px", borderRadius: "50px", background: isPending ? "#fef9c3" : isVerified ? "#dcfce7" : "#fee2e2", color: isPending ? "#92400e" : isVerified ? "#16a34a" : "#dc2626" }}>
                      ...
                      <div style={{ background: "var(--gray-50)", borderRadius: "10px", padding: "14px 16px", marginBottom: "16px", display: "flex", gap: "24px", flexWrap: "wrap", alignItems: "center", border: "1px solid var(--gray-100)" }}>
                        <div>
                          <p style={{ fontSize: "11px", color: "var(--gray-400)", marginBottom: "2px" }}>Transaction ID</p>
                          <p style={{ fontSize: "15px", fontWeight: "700", color: "var(--gray-800)" }}>{b.transactionId}</p>
                        </div>
                        {b.paymentScreenshot && (
                          <div>
                            <p style={{ fontSize: "11px", color: "var(--gray-400)", marginBottom: "4px" }}>Screenshot</p>
                            <img src={b.paymentScreenshot} alt="Payment proof" onClick={() => setPreviewScreenshot(b.paymentScreenshot)}
                              style={{ width: "90px", height: "66px", objectFit: "cover", borderRadius: "8px", border: "2px solid var(--gray-200)", cursor: "pointer", transition: "all 0.2s" }}
                              onMouseEnter={e => { e.target.style.borderColor = "#16a34a"; e.target.style.transform = "scale(1.05)"; }}
                              onMouseLeave={e => { e.target.style.borderColor = "var(--gray-200)"; e.target.style.transform = "scale(1)"; }}
                      ...
                      {isPending && (
                        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "center" }}>
                          <input type="text" placeholder="Add a note (optional)..." value={noteTarget === b._id ? adminNote : ""} onChange={e => { setNoteTarget(b._id); setAdminNote(e.target.value); }} onFocus={() => setNoteTarget(b._id)}
                          ...
                          />
                          <button onClick={() => verifyPayment(b._id, "approve")} disabled={verifyingId === b._id} style={{ background: verifyingId === b._id ? "#86efac" : "#16a34a", color: "white", border: "none", padding: "10px 22px", borderRadius: "50px", fontSize: "13px", fontWeight: "600", cursor: verifyingId === b._id ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}>
                            {verifyingId === b._id ? "Processing..." : "✅ Approve"}
                          </button>
                          <button onClick={() => verifyPayment(b._id, "reject")} disabled={verifyingId === b._id} style={{ background: "none", color: "#dc2626", border: "1.5px solid #fecaca", padding: "10px 22px", borderRadius: "50px", fontSize: "13px", fontWeight: "600", cursor: verifyingId === b._id ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}
                            onMouseEnter={e => { if (verifyingId !== b._id) { e.currentTarget.style.background = "#dc2626"; e.currentTarget.style.color = "white"; } }}
                            onMouseLeave={e => { if (verifyingId !== b._id) { e.currentTarget.style.background = "none"; e.currentTarget.style.color = "#dc2626"; } }}
                          >❌ Reject</button>
                        </div>
                      )}
                    </div>
        ...
        {activeTab === "tours" && (
          <div>
            <div style={{ background: "white", border: "1px solid var(--gray-100)", borderRadius: "16px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
              <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--gray-100)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
                <div>
                  <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "18px", fontWeight: "700", color: "var(--gray-800)", marginBottom: "4px" }}>All Tours</h3>
                  <p style={{ fontSize: "13px", color: "var(--gray-400)" }}>{tours.length} tour{tours.length !== 1 ? "s" : ""} · Add, edit or delete directly</p>
                </div>
                <button onClick={openAddTour} style={{ background: "#16a34a", color: "white", border: "none", padding: "10px 22px", borderRadius: "50px", fontSize: "13px", fontWeight: "600", cursor: "pointer", transition: "all 0.2s", boxShadow: "0 4px 12px rgba(22,163,74,0.3)", display: "flex", alignItems: "center", gap: "7px" }}
              ...
              {tours.length === 0 ? (
                <div style={{ textAlign: "center", padding: "60px 0" }}>
                  ...
                  <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "20px", color: "var(--gray-800)", marginBottom: "8px" }}>No tours yet</h3>
                  <button onClick={openAddTour} style={{ background: "#16a34a", color: "white", border: "none", padding: "12px 28px", borderRadius: "50px", fontSize: "14px", fontWeight: "600", cursor: "pointer" }}>🌿 Add New Tour</button>
                </div>
              ) : (
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                      {tours.map((t) => (
                        <tr key={t._id} onMouseEnter={e => e.currentTarget.style.background = "var(--gray-50)"} onMouseLeave={e => e.currentTarget.style.background = "white"}>
                          <td style={tdStyle}>
                            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                              {(t.images?.[0] || t.photo) && <img src={t.images?.[0] || t.photo} alt={t.tourName} style={{ width: "44px", height: "36px", objectFit: "cover", borderRadius: "6px", flexShrink: 0 }} />}
                              <div>
                                <p style={{ fontWeight: "600", color: "var(--gray-800)", fontSize: "13px" }}>{t.tourName || t.title || t.name}</p>
                                {t.duration && <p style={{ fontSize: "11px", color: "var(--gray-400)", marginTop: "1px" }}>🕐 {t.duration}</p>}
                              </div>
                            </div>
                          </td>
          ...
          <div style={{ background: "white", border: "1px solid var(--gray-100)", borderRadius: "16px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {bookings.length === 0 ? (
                    <tr><td colSpan="8" style={{ ...tdStyle, textAlign: "center", padding: "40px", color: "var(--gray-400)" }}>No bookings yet</td></tr>
                  ) : bookings.map((b) => {
                    const status = (b.status || "pending").toLowerCase();
                    const sc = STATUS_COLORS[status] || STATUS_COLORS.pending;
                    const pm = PAYMENT_METHOD_INFO[b.paymentMethod] || {};
                    return (
                      <tr key={b._id} onMouseEnter={e => e.currentTarget.style.background = "var(--gray-50)"} onMouseLeave={e => e.currentTarget.style.background = "white"}>
                        ...
                        <td style={tdStyle}>
                          <select value={b.status || "Pending"} onChange={e => updateBookingStatus(b._id, e.target.value)} style={{ padding: "5px 10px", border: "1.5px solid var(--gray-200)", borderRadius: "8px", fontSize: "12px", fontFamily: "'DM Sans', sans-serif", color: "var(--gray-700)", background: "white", cursor: "pointer", outline: "none" }}>
          ...
          <div style={{ background: "white", border: "1px solid var(--gray-100)", borderRadius: "16px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid var(--gray-100)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
              <div>
                ...
                <p style={{ fontSize: "13px", color: "var(--gray-400)" }}>Delete reviews or block users from submitting new ones.</p>
              </div>
            ...
            {allReviews.length === 0 ? (
              ...
              const userId    = review.userId?._id || review.user || review.userId;
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px", flexShrink: 0 }}>
                    ...
                    <button onClick={() => toggleBlockUser(userId, userName)} style={{ background: isBlocked ? "#dcfce7" : "#fef9c3", color: isBlocked ? "#16a34a" : "#ca8a04", border: `1.5px solid ${isBlocked ? "#86efac" : "#fde68a"}`, padding: "7px 16px", borderRadius: "50px", fontSize: "12px", fontWeight: "600", cursor: "pointer", whiteSpace: "nowrap" }}>
